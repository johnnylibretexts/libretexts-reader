...
**App data** lives outside the repo at `~/Library/Application Support/dev.johnnyrobot.reader` (SQLite DB, downloaded books, TTS models, cover/section images). Point tests/local runs elsewhere with `JOHNNY_READER_APP_DATA_DIR`.
...
Pre-commit/verification gate that was green at last handoff: `npm run build`, `cargo test -p johnny-reader`, `git diff --check`.

## Conventions

- **Toolchains are pinned.** Rust: stable via rustup (`rust-toolchain.toml` adds `clippy`+`rustfmt`; workspace `rust-version = 1.88`). Use `cargo fmt` / `cargo clippy`. Frontend has no separate linter — TypeScript strictness via `tsc` in `npm run build` is the gate.
