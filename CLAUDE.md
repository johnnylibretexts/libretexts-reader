...
**TTS runs in Rust, behind a `TtsProvider` trait** (`src-tauri/src/tts/provider.rs`), mirroring `SpeechEngine` in `src/lib/speech/types.ts` — same idea, two layers. Two implementations today: **Supertonic** (on-device, `Rust ONNX Runtime` / `ort`, on-demand model downloads) and **Fish Audio** (cloud, plain HTTP via `reqwest`, not the webview). Chapter export encodes locally: **AAC/M4A via macOS AudioToolbox** for Supertonic (ADR-0004); Fish returns MP3 from its API, so the two providers ship different containers and anything naming or tagging an export must read `export_extension`. Kokoro was removed in favour of Supertonic (ADR-0003).
...
- **Four declarations carry the identifier string and nothing in the code links them:** `identifier` in `tauri.conf.json`, `APP_DIR_NAME` in `src-tauri/src/paths.rs`, the `assetProtocol` `$APPDATA/**` scope, and `KEYCHAIN_SERVICE` in `src-tauri/src/secrets.rs`. Tauri derives `$APPDATA` from the identifier. If the first three drift the build stays green and every cover and figure silently stops rendering. **`KEYCHAIN_SERVICE` drifting has no symptom at all** — the app builds, runs and renders, and simply looks for the reader's Fish Audio key under a service name nothing writes any more, so the key appears to have vanished and they are asked to enter it again. `scripts/ci/check-identifier.sh` gates all four — keep it passing, and never change one of them alone.
