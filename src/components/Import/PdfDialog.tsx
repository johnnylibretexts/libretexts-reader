...
export function PdfDialog({ onImported }: PdfDialogProps) {
  ...
  async function importFile() {
    ...
    setStage("Opening...");
    const timers = [
      window.setTimeout(() => setStage("Extracting..."), 500),
      window.setTimeout(() => setStage("Saving..."), 1200),
    ];

    try {
      const documentId = await api.importPdf(filePath);
  ...
  return (
    <section className="flex flex-col gap-4 rounded-md border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-base font-semibold">PDF Import</h2>
      </div>
